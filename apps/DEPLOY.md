# Deploying the BrainHalf studio apps

> **This file is about `apps/` only** — the AI coding studio. It is NOT about the
> document-extraction platform that serves brainhalf.com; that one is deployed
> from the repository root and documented in `../DEPLOY.md`.
>
> It used to sit at the repository root titled "Deploying BrainHalf on
> Cloudflare", which made it look like the deployment guide for the whole repo.
> Following it from there would have published this app over brainhalf.com.
>
> `apps/` and `packages/` are outside the pnpm workspace (see `../README.md`), so
> the commands below need their own install before they will run.
